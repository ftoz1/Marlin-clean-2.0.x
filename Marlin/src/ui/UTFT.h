...
#define RM68140			31	// ILI9486	(16bit)
...
extern void LCD_IO_WriteMultiple32(uint32_t data, uint32_t count);
extern void LCD_IO_WriteSequence(uint16_t *data, uint32_t length);
extern void LCD_IO_WriteSequence32(uint16_t *data, uint32_t length);
extern void FSMC_Init(uint8_t cs, uint8_t rs);*/
...
class UTFT
{
	public:
		//UTFT();
		UTFT(TFT_FSMC *ptrIO, byte model, int RS, int WR, int CS, int RST, int SER=0);
        ...
        void	drawrect1(int x1, int y1, int x2, int y2,word col);     //add
		...
		byte	printNumI(long num, int x, int y, int length=0, char filler=' ');
		void	printNumF(double num, byte dec, int x, int y, char divider='.', int length=0, char filler=' ');
		void	setFont(uint8_t* font);
		uint8_t* getFont();
		uint8_t	getFontXsize();
		uint8_t	getFontYsize();
        void    drawBitmapToDark(int x, int y, int sx, int sy, bitmapdatatype data, int iterance, int scale=1);
        void    drawBitmapR(int x, int y, int sx, int sy, bitmapdatatype data, int rot=0);
    	void	drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int scale=1);
		void	drawBitmap(int x, int y, int sx, int sy, bitmapdatatype data, int deg, int rox, int roy);
        void    screenOrient(byte ort);
		void	lcdOff();
		void	lcdOn();
...
inline  void LCD_Write_COM(word com);
inline  void LCD_Write_DATA(word dat);
inline  void fast_fill_16(word color,long length);
inline  void fast_fill_32(word color,long length);
inline 	void LCD_Write_COM_DATA(char com1,int dat1);
